"""
Solara AI Quant — Pull Back Entry SHORT Feature Engineer
=========================================================

Computes the full feature set for the Pull Back Entry SHORT model (entry_H1_short.joblib).

This is a 3-stage pipeline:
  Stage 1 — Trend Alignment   : W1 + D1 + H4 trend models (≥ 2/3 must agree on DOWNTREND)
  Stage 2 — Pullback Detector : H4 SHORT pullback model (class-2 prob ≥ 0.65)
  Stage 3 — Entry Features    : 37-feature H1 vector for the SHORT entry model

Trigger TF: H1
merge_timeframes: []  — this FE loads H4 / D1 / W1 CSVs directly.

Key differences from PullBackFeatureEngineer (Long):
  - Pullback model: pull_back_pullback_h4_short.joblib
  - Fib levels measured from swing_LOW upward (bear pullback retracements)
  - Pullback model features use dist_to_fib382_short / dist_to_fib618_short
  - Trend model path: Pull Back Strategy Short / models /

Output columns (per symbol, latest H1 bar):
  - All 37 ENTRY_FEATURES (identical to Long — same entry model architecture)
  - _pb_trend_aligned  (bool)  — ≥ 2/3 trend models agree on DOWNTREND
...
Notes
-----
- Fibonacci levels for SHORT: fib_level = swing_low + fib * impulse_range (upward from low).
  This is the mirror image of Long (which is swing_high - fib * impulse_range downward).
- The 37 entry features are identical to Long — the SHORT entry model was trained with the
  same feature set. Both long-side and short-side BB features are included.
- Trend models are loaded from the Pull Back Strategy SHORT models directory (same files as Long
  — copied from Long project, not retrained).
"""
...
try:
    ...
    _TI_AVAILABLE = True
    logger.info("[PBShortFE] Trend Identifier package loaded")
except ImportError as _e:
    _TI_AVAILABLE = False
    logger.critical(
        f"[PBShortFE] Cannot import forex_trend_model: {_e}. "
...
_PULLBACK_SHORT_MODEL_PATH = MODELS_DIR / 'pull_back_pullback_h4_short.joblib'
...
# ── Entry model feature list (37 features — identical to Long, must match training) ──
ENTRY_FEATURES = [
    # ── H1 CSV features (25) ──────────────────────────────────────────────────
...
class PullBackShortFeatureEngineer(BaseFeatureEngineer):
    """
    Feature engineer for the Pull Back Entry SHORT model.

    Mirrors PullBackFeatureEngineer but adapted for the SHORT (bearish) direction:
      - Loads the SHORT pullback model (pull_back_pullback_h4_short.joblib)
      - Computes Fibonacci distances from swing_LOW upward (bear pullback retracements)
      - Uses dist_to_fib382_short / dist_to_fib618_short in the pullback model feature set
      - Trend models are the same pre-trained files (copied from Long project)

    All 37 entry features are identical to the Long FE — the SHORT entry model was
    trained on the same feature architecture, filtered to downtrend rows only.
    """

    # ── Class-level caches (separate from Long FE — different class) ──────────
    _trend_predictors: dict   = {}    # tf -> LiveTrendPredictor
    _pb_short_model           = None
    _pb_short_feature_cols: list = []
    _pb_short_threshold: float   = 0.65
    ...
    def compute(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not _TI_AVAILABLE:
            logger.error("[PBShortFE] Trend Identifier not available — skipping")
            return pd.DataFrame()

        if PullBackShortFeatureEngineer._pb_short_model is None:
            logger.error("[PBShortFE] Pullback SHORT model not loaded — skipping")
            return pd.DataFrame()
        ...
        if h4_all is None:
            logger.error("[PBShortFE] H4 CSV unavailable — skipping")
            return pd.DataFrame()
        ...
        for sym in symbols:
            try:
                ...
                logger.error(f"[PBShortFE] {sym} failed: {exc}", exc_info=True)

        if not rows:
            logger.warning(
                f"[PBShortFE] compute() produced 0 rows from {len(symbols)} symbols "
        ...
        for col in ENTRY_FEATURES:
            if col not in result.columns:
                logger.warning(f"[PBShortFE] Missing output feature '{col}' — filling 0")
                result[col] = 0.0
        return result
    ...
    def _compute_for_symbol(
        ...
        if h4 is None or len(h4) < _MIN_H4_BARS:
            logger.debug(f"[PBShortFE] {sym}: insufficient H4 bars ({len(h4) if h4 is not None else 0})")
            return None
        ...
        if h4_trend_pred is None:
            logger.debug(f"[PBShortFE] {sym}: H4 trend prediction failed")
            return None
        ...
        # ── 6. Run SHORT pullback model on last N H4 bars ──────────────────
        ...
        # Problem: after a sharp H4 drop, price bounces (pullback). The H4
        # pullback model labels early bounce bars as "exhaustion" — but the
        # bounce is still printing bullish H4 candles. Entering SHORT while
        # H4 is still green means fighting the live bounce momentum, resulting
        # in repeated SL hits before the bounce actually reverses.
        #
        # Gate: the most recently closed H4 bar must be BEARISH (close < open).
        # A bearish H4 bar confirms the bounce is losing steam and rolling over.
        # If H4 is still printing bullish bars, the pullback is not yet exhausted
        # — wait for at least one H4 reversal bar before entering SHORT.
        if trend_aligned and trend_direction == 'downtrend':
            latest_h4_bar = h4.iloc[-1]
            h4_close = float(latest_h4_bar.get('close', 0))
            h4_open  = float(latest_h4_bar.get('open',  0))
            if h4_close >= h4_open:
                logger.debug(
                    f"[PBShortFE] {sym}: H4 momentum gate — latest H4 bar is bullish "
                    f"(close={h4_close:.5f} >= open={h4_open:.5f}) "
                    f"— bounce still in progress, blocking SHORT entry"
                )
                trend_aligned   = False
                trend_direction = 'sideways'
        ...
        logger.debug(
            f"[PBShortFE] {sym}: pullback — label={_label_name}({_label})  "
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # H4 pullback features — SHORT version
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def _compute_h4_pullback_features(
        ...
        SHORT version: Fibonacci levels measured from swing_LOW upward.
        For bear pullbacks, price bounces from the swing low; retracement
        levels are: fib_level_short = swing_low + fib * impulse_range.
        Feature names: dist_to_fib382_short, dist_to_fib618_short.
        """
        df = h4.copy()
        ...
        # SHORT Fibonacci distance — from swing_low UPWARD (bear pullback retracements)
        # At 38.2%: price retraced 38.2% of the impulse from low → fib = swing_low + 0.382 * range
        # At 61.8%: price retraced 61.8% of the impulse from low → fib = swing_low + 0.618 * range
        for fib, label in [(0.382, '382'), (0.618, '618')]:
            fib_level_short = df['swing_low'] + fib * df['impulse_range']
            df[f'dist_to_fib{label}_short'] = (df['close'] - fib_level_short).abs() / df['impulse_range']
    ...
    def _run_pullback_model(self, h4: pd.DataFrame, last_n: int = 30) -> pd.DataFrame:
        ...
        pb_feats = [
            ...
            'close_location', 'upper_wick_ratio', 'lower_wick_ratio',
            'dist_to_fib382_short', 'dist_to_fib618_short',   # ← SHORT: from swing_low up
        ...
        try:
            probs = PullBackShortFeatureEngineer._pb_short_model.predict_proba(X)
        except Exception as exc:
            logger.error(f"[PBShortFE] Pullback SHORT model predict_proba failed: {exc}")
        ...
        threshold = PullBackShortFeatureEngineer._pb_short_threshold
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # H1 candle features (identical to Long FE — same 37-feature set)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def _compute_h1_features(self, h1: pd.DataFrame) -> dict:
        ...
        Note: Even for the SHORT model, the 37 entry features include long-side BB
        features. This is intentional — the SHORT entry model was trained on the
        same 37-feature set as the Long model (both sides provide context).
        """
        df = h1.copy()
    ...
    def _get_trend_pred(
        ...
        predictor = PullBackShortFeatureEngineer._trend_predictors.get(tf)
        ...
        try:
            ...
            logger.error(f"[PBShortFE] Trend model {tf} failed for {sym}: {exc}", exc_info=True)
            return None
    ...
    def _load_tf_csv(self, tf: str) -> Optional[pd.DataFrame]:
        ...
        if tf_cfg is None:
            logger.warning(f"[PBShortFE] No TIMEFRAMES config for {tf}")
            return None

        csv_path = MQL5_FILES_DIR / tf_cfg.csv_filename
        if not csv_path.exists():
            logger.warning(f"[PBShortFE] {tf} CSV not found: {csv_path}")
            return None
        ...
        try:
            ...
            logger.error(f"[PBShortFE] Failed to load {tf} CSV: {exc}")
            return None
    ...
    def _load_pullback_model(self):
        if PullBackShortFeatureEngineer._pb_short_model is not None:
            return
        if not _PULLBACK_SHORT_MODEL_PATH.exists():
            logger.error(f"[PBShortFE] Pullback SHORT model not found: {_PULLBACK_SHORT_MODEL_PATH}")
            return
        try:
            pkg = joblib.load(_PULLBACK_SHORT_MODEL_PATH)
            PullBackShortFeatureEngineer._pb_short_model        = pkg['model']
            PullBackShortFeatureEngineer._pb_short_feature_cols = pkg.get('feature_cols', [])
            PullBackShortFeatureEngineer._pb_short_threshold    = float(pkg.get('threshold', 0.65))
            logger.info(
                f"[PBShortFE] Pullback SHORT model loaded — "
                f"threshold={PullBackShortFeatureEngineer._pb_short_threshold} "
                f"features={len(PullBackShortFeatureEngineer._pb_short_feature_cols)}"
            )
        except Exception as exc:
            logger.error(f"[PBShortFE] Failed to load pullback SHORT model: {exc}")

    def _load_trend_models(self):
        if not _TI_AVAILABLE:
            return
        for tf, model_path in _SHORT_TREND_MODEL_PATHS.items():
            if tf in PullBackShortFeatureEngineer._trend_predictors:
                continue
            if not model_path.exists():
                logger.warning(f"[PBShortFE] Trend model {tf} not found: {model_path}")
                continue
            try:
                predictor = LiveTrendPredictor.from_package(str(model_path))
                PullBackShortFeatureEngineer._trend_predictors[tf] = predictor
                logger.info(f"[PBShortFE] Trend model {tf} loaded from {model_path}")
            except Exception as exc:
                logger.error(f"[PBShortFE] Failed to load trend model {tf}: {exc}")
