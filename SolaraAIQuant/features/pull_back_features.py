...
class PullBackFeatureEngineer(BaseFeatureEngineer):
    ...
    def _check_alignment(
        ...
        """
        Check trend alignment for LONG entries.

        Two-part filter:
          1. D1 veto: if D1 confidently says DOWNTREND (prob_down ≥ 0.55), block
             immediately — trading LONG against a clear D1 downtrend is wrong.
        ...
        D1_VETO_CONFIDENCE   = 0.55   # D1 blocks LONG if prob_down exceeds this
        ...
        for tf, pred in (('H4', h4_pred), ('D1', d1_pred), ('W1', w1_pred)):
            if pred is not None and pred['model_valid'].any():
                ...
                if cls == 'uptrend' and up >= MIN_TREND_CONFIDENCE:
                    tf_votes[tf] = 'uptrend'
                elif cls == 'downtrend' and dn >= MIN_TREND_CONFIDENCE:
        ...
        # ── Gate A: D1 veto ───────────────────────────────────────────────────
        # Block LONG if D1 is confidently downtrend. D1 sideways/uncertain = neutral.
        d1_vote = tf_votes.get('D1', 'missing')
        if d1_vote == 'downtrend':
            logger.debug(
                f"[PullBackFE] {sym}: alignment — D1 veto BLOCKED LONG "
                f"(D1 confidently downtrend ≥{D1_VETO_CONFIDENCE})  [{tf_summary}]"
            )
        ...
        # Also count D1 if it has a confident uptrend vote (adds weight)
        if d1_vote == 'uptrend':
            confident_votes.append('uptrend')

        if len(confident_votes) < 2:
            logger.debug(
                f"[PullBackFE] {sym}: alignment — fewer than 2 confident TF votes  [{tf_summary}]"
            )
            return False, 'sideways'
        ...
        if up_count >= 2:
            logger.debug(f"[PullBackFE] {sym}: alignment — UPTREND ✓ (confident 2/3 + D1 veto passed)  [{tf_summary}]")
            return True, 'uptrend'
        if down_count >= 2:
            logger.debug(f"[PullBackFE] {sym}: alignment — DOWNTREND ✓ (confident 2/3)  [{tf_summary}]")
