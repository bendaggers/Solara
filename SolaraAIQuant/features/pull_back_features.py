...
class PullBackFeatureEngineer(BaseFeatureEngineer):
    ...
    def _check_alignment(
        ...
        Two-part filter:
          1. D1 mandatory: D1 must classify as UPTREND with confidence ≥
             MIN_D1_CONFIDENCE. If D1 is downtrend/sideways or uncertain, block
          ...
          2. Confident 2/3 vote: each TF vote only counts if the winning
             ...
             (e.g. prob_up=0.42) are treated as abstentions rather than votes.
        ...
        # ── Gate A: D1 mandatory ──────────────────────────────────────────────
        # D1 must be a confident UPTREND. If D1 is downtrend, sideways, uncertain,
        # or missing — block. Prevents trading against the structural daily trend.
        d1_vote = tf_votes.get('D1', 'missing')
        if d1_vote != 'uptrend':
            logger.debug(
                f"[PullBackFE] {sym}: alignment — D1 mandatory gate FAILED "
                f"(D1={d1_vote}, need confident uptrend ≥{MIN_D1_CONFIDENCE})  [{tf_summary}]"
            )
        ...
        if up_count >= 2:
            logger.debug(f"[PullBackFE] {sym}: alignment — UPTREND ✓ (D1 mandatory + confident 2/3)  [{tf_summary}]")
            return True, 'uptrend'
        if down_count >= 2:
