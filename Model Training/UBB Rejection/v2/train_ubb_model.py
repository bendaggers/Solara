...
def summarize_walk_forward(wf_df: pd.DataFrame) -> Dict[str, float]:
    ...
    # The walk-forward dataframe is wide/sparse:
    # each row only has values populated for its own split columns
    # (e.g. wf1_selected_pf on row 1, wf2_selected_pf on row 2, etc.).
    # So we must aggregate across all matching columns and drop NaNs.
    pf_values = []
    exp_values = []
    tr_values = []

    for col in wf_df.columns:
        if col.endswith("_selected_pf"):
            pf_values.extend(pd.to_numeric(wf_df[col], errors="coerce").dropna().tolist())
        elif col.endswith("_selected_expectancy"):
            exp_values.extend(pd.to_numeric(wf_df[col], errors="coerce").dropna().tolist())
        elif col.endswith("_selected_trades"):
            tr_values.extend(pd.to_numeric(wf_df[col], errors="coerce").dropna().tolist())

    pf = pd.Series(pf_values, dtype="float64")
    exp = pd.Series(exp_values, dtype="float64")
    tr = pd.Series(tr_values, dtype="float64")

    return {
        "wf_splits": int(len(pf)),
        "wf_mean_pf": round(float(pf.mean()), 4) if not pf.empty else np.nan,
        "wf_median_pf": round(float(pf.median()), 4) if not pf.empty else np.nan,
        "wf_mean_expectancy": round(float(exp.mean()), 4) if not exp.empty else np.nan,
        "wf_mean_selected_trades": round(float(tr.mean()), 2) if not tr.empty else np.nan,
        "wf_positive_pf_splits": int((pf > 1.0).sum()) if not pf.empty else 0,
    }
